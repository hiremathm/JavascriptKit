...
class TableRow extends React.Component {
  ...
  clickTicket = () => {
    let priorityHandle = document.getElementById(this.props.ticket.id)
    console.log(priorityHandle.innerHTML)
  ...
  updatePriority = (e) => {
    let priorityHandle = e.target.value
    let url = `http://dct-api-data.herokuapp.com/tickets/${this.props.ticket.ticket_code}?api_key=5cc7efcde767745c`
  ...
  render(){
    return(
      <tr key={this.props.ticket.id}>
        ...
        <button id={this.props.ticket.id} onClick = {this.clickTicket} >Update Priority</button> <button id={this.props.ticket.id} onClick = {this.removeTicket} value="remove">Remove</button></td>
      </tr>
    )
  }
}
